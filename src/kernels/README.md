...
where the reciprocal standard deviation ($\text{rstd}$) is defined as:
...
$$
\hat{X} = \text{RMS}(S) = S \cdot \sigma^{-1}, \qquad Y = W \odot \hat{X}
$$
...
$$
\frac{\partial L}{\partial X} = dS \cdot \frac{\partial S}{\partial X} = dS, \qquad \frac{\partial L}{\partial R} = dS \cdot \frac{\partial S}{\partial R} = dS
$$
...
**3. Soft-Clamping (Activation Capping)**
...
$$
\frac{\partial \text{LSE}_t}{\partial z_{t, v}} = \frac{\partial}{\partial z_{t, v}}\left( m_t + \log \sum_{k=1}^V \exp(z_{t, k} - m_t) \right) = \frac{\exp(z_{t, v} - m_t)}{\sum_{k=1}^V \exp(z_{t, k} - m_t)} = P_{t, v}
$$
